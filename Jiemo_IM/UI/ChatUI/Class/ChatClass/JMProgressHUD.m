...
+ (instancetype)sharedView
{
    ...
    dispatch_once(&once, ^ {
        sharedView = [[JMProgressHUD alloc] initWithFrame:[UIScreen screenBounds]];
...
+ (void)show {
    [[JMProgressHUD sharedView] show];
}

+ (void)changeSubTitle:(NSString *)str
{
    [JMProgressHUD sharedView].subTitleLabel.text = str;
}

+ (void)dismissWithSuccess:(NSString *)str
{
    [[JMProgressHUD sharedView] dismiss:str];
}
...
- (void)show
{
    ...
    self.titleLabel.center = CGPointMake([UIScreen screenWidth]/2,[UIScreen screenHeight]/2 - 30);
    self.titleLabel.text = @"Time Limit";
    
    self.subTitleLabel.center = CGPointMake([UIScreen screenWidth]/2,[UIScreen screenHeight]/2 + 30);
    self.subTitleLabel.text = @"Slide up to cancel";
    
    self.centerLabel.center = CGPointMake([UIScreen screenWidth]/2,[UIScreen screenHeight]/2);
...
{
    if (!_edgeImageView) {
        _edgeImageView = [[UIImageView alloc] initWithImage:[UIImage imageWithName:@"Chat_record_circle"]];
